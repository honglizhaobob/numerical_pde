...
disp("=== displaying external pressure ===")
...
U(1,:) = A;
U(2,:) = Q;
maxflow = 0.0; % records Q_max at x=N/2, update every time loop
maxflow_init = 0; % not initialized
% ==============================
%       Main loop solver
% ==============================
for n = 1:length(t)-1
    ...
    for i = 1:length(x)
        disp('==========*==========*==========*==========')
        disp('=             Reporting                   =')
        disp(['- At (x,t) position: ','(',num2str(n),',',...
            num2str(i),')'])
...
Qtru = Utru(2);
f = [Qtru; 
    (alpha*Qtru^2/Atru)+(G0/3/rho*((Atru/A0)^(3/2))*A0)];
end

%calculate the RHS
function s = rhs(Utru) 
% returns the source term [0; f(A,Q)] on the RHS
global alpha nu 
Atru = Utru(1);
Qtru = Utru(2);
